...
def apply_theme(root: tk.Tk | tk.Toplevel) -> None:
    ...
    style.configure("Small.TButton", background=COLORS["panel_alt"], foreground=COLORS["muted"], padding=(8, 4), font=("Segoe UI", 9))
    style.map("Small.TButton", background=[("active", COLORS["border"]), ("disabled", "#172234")], foreground=[("disabled", "#9aa6b5")])
    style.configure("CompactStatSection.TLabel", background=COLORS["panel"], foreground=COLORS["muted"], font=("Segoe UI", 9, "bold"))
    style.configure("CompactStatValue.TLabel", background=COLORS["panel"], foreground=COLORS["text"], font=("Segoe UI", 12, "bold"))
    style.configure("TSeparator", background=COLORS["border"])
    style.configure("FlatLead.TLabel", background=COLORS["bg"], foreground=COLORS["text"], font=("Segoe UI", 11, "bold"))
    style.configure("FlatMuted.TLabel", background=COLORS["bg"], foreground=COLORS["muted"])
    style.configure("FlatStatSection.TLabel", background=COLORS["bg"], foreground=COLORS["muted"], font=("Segoe UI", 9, "bold"))
    style.configure("FlatStatValue.TLabel", background=COLORS["bg"], foreground=COLORS["text"], font=("Segoe UI", 12, "bold"))
